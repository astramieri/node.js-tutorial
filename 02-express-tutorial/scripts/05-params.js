...
app.get("/api/v1/query", (req, res) => {
  ...
  // WARN: return clause (you can't send two response!)
  if (sortedProducts.length < 1) {
    //res.status(200).send("no products matched your search");
    return res.status(200).json({ success: true, data: [] }); // WARN: return
  }

  return res.status(200).json(sortedProducts); // RETURN: best practices?
});
